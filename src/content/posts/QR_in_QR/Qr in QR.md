...
### 2.2 Các phiên bản và dung lượng
...
Trong Nested QR-in-QR, scanner tìm được Finder Pattern của cả outer lẫn inner. Nhưng vì inner QR nhỏ và nằm ở giữa ảnh, 3 Finder Pattern của nó nằm gần nhau — scanner đủ 3 FP của inner tại row ~179. Outer QR trải rộng toàn ảnh, FP thứ 3 nằm tận row ~238. Lúc này tùy vào nhiều yếu tố như thư viện áp dụng, phương pháp xử lý mà scanner sẽ có các hướng xử lý:

- Trường hợp 1: 1 vài scanner được thiết kế để quét càng nhanh càng tốt. Khi thấy đủ 3 FP hợp lệ (đủ điều kiện hình học và kích thước) rồi sẽ tiến hành decode ngay lập tức. Đây là trường hợp tệ nhất, scanner sẽ decode inner QR trước và cho ra kết quả dẫn tới false positive.

- Trường hợp 2: Một số hệ thống quét sâu sẽ quét toàn ảnh chứ không dừng sớm. Mỗi khi gặp 1 FP nó sẽ lưu lại vị trí sau khi quét toàn bộ mới ghép thành qr hoàn chỉnh và decode. Lúc này lại tùy thuộc vào cách implement và ưu tiên (kích thước, vị trí, độ tin cậy) để trả về có thể là inner, outer hoặc cả 2 cùng lúc. 

Hầu hết các hệ thống hiện nay đều rơi vào trường hợp 1. Các thư viện mã nguồn mở như ZXing thường được thiết kế cho tác vụ quét thời gian thực (real-time video), nên ưu tiên tối ưu tốc độ và giảm độ trễ bằng cơ chế dừng sớm.

Tuy nhiên, ngay cả khi các hệ thống áp dụng trường hợp 2, việc quét toàn bộ ảnh và xử lý nhiều QR candidate có thể dẫn đến các kết quả không nhất quán. Tùy vào cách implement, hệ thống có thể trả về QR đầu tiên được decode thành công (thường là Inner QR), ưu tiên một QR theo tiêu chí nội bộ (kích thước, vị trí, độ tin cậy) hoặc trả về nhiều QR cùng lúc.

Sự không nhất quán này có thể tạo ra trạng thái mơ hồ trong quá trình phân tích, đặc biệt đối với các hệ thống email gateway hoặc security scanner, từ đó làm tăng khả năng đưa ra quyết định phân loại sai đối với nội dung chứa QR code. Vì vậy xác suất để kĩ thuật trên thành công tương đối cao.

**Camera điện thoại:** 

Camera điện thoại hiện đại cũng có năng lực phần cứng đủ mạnh để phát hiện và duy trì nhiều mã QR trong cùng một khung hình. Việc nạn nhân bị lừa hiển thị URL độc hại dựa trên các tiêu chí ưu tiên nội bộ của ứng dụng quét mã.

Do mã nguồn của các ứng dụng Camera hay các ứng dụng quét mã như Zalo là mã nguồn đóng, chúng ta không thể xác định chính xác thuật toán xếp hạng bên dưới. Tuy nhiên, thông qua phân tích hộp đen (black-box testing) và quan sát hành vi, kịch bản tấn công được diễn giải như sau:

- Sự thao túng từ khung ngắm: Ứng dụng thường hiển thị một khung định hướng. Theo phản xạ, người dùng sẽ lùi điện thoại để canh cho toàn bộ viền mã lớn lọt vừa vặn vào khung ngắm này.

- Hành vi ưu tiên hiển thị: Tại khoảng cách này, dù cả hai mã đều có khả năng được hệ thống nhận diện, ứng dụng Camera sẽ ưu tiên hiển thị kết quả của Outer QR. Dựa trên biểu hiện thực tế, cơ chế ưu tiên nội bộ của các ứng dụng này có xu hướng thiên vị các mã QR có diện tích hiển thị áp đảo hoặc khớp với khung định hướng của giao diện.

- Quá trình trích xuất mã độc: Khi phần mềm đã khóa mục tiêu vào Outer QR, nó đối mặt với việc vùng lõi bị Inner QR ghi đè. Lúc này, bộ giải mã tự động kích hoạt thuật toán nội suy Reed-Solomon (ECC Level H) để khôi phục 30% dữ liệu bị khuyết, qua đó giải mã thành công URL Phishing.

- Kịch bản ngoại lệ: Nếu người dùng dí sát điện thoại vào lõi mã khiến các điểm định vị của Outer QR văng ra khỏi khung hình hay khung định vị, Outer QR sẽ bị loại bỏ và ứng dụng buộc phải hiển thị kết quả của Inner QR. Tuy nhiên, trong thực tế, người dùng thường căn chỉnh để mã lớn vừa khít khung quét thay vì dí camera lại gần, nên phần lớn trường hợp sẽ rơi vào dải khoảng cách mà Outer QR được phát hiện ổn định, khiến kỹ thuật này có tỷ lệ thành công cao.
...
- **Kết quả:** Như đã nói trước đó, nếu giơ điện thoại ở khoảng cách xa hoặc đủ cho outer lọt khung định hường thì với khả năng sửa lỗi ECC Level H, điện thoại bỏ qua phần nhiễu ở lõi và nhận diện mã lớn bên ngoài. Một Pop-up hiện lên chuyển hướng bạn thẳng tới trang phishing. Thực tế nếu dí sát điện thoại vô thì khả năng vẫn bắt được qr ở giữa nhưng hầu hết sẽ bắt cả cái qr to bên ngoài thôi.
